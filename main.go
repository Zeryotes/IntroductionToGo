...
func player1(state chan int) {
	...
	// Seed para garantir aleatoriedade entre as jogadas
...
func startGame(channel chan int) {
	...
	for i := 1; i <= MATCH; i++ {
		...
		// Cada novo MATCH reinicia os valores de SET de cada PLAYER
		SETPLAYER1 = 0
		SETPLAYER2 = 0
		for j := 1; j <= SET; j++ {
			fmt.Printf("SET %d STARTED!!!\n", j)
			// Cada novo SET reinicia os valores de GAME de cada PLAYER
			GAMEPLAYER1 = 0
			GAMEPLAYER2 = 0
			for k := 1; k <= GAME; k++ {
				fmt.Printf("GAME %d STARTED!!!\n", k)
				// Cada novo GAME reinicia os SCORES de cada PLAYER
				SCOREPLAYER1 = 0
				SCOREPLAYER2 = 0
				for {
					// Continua chamando goroutinas até um dos PLAYERS ganharem o GAME
				...
				// Verifica quem ganhou o SET após uma sequencia de GAMES
			...
			// Verifica quem ganhou a MATCH após uma sequencia de SET's
		...
		if MATCHPLAYER1 >= (MATCH/2)+1 {
			break
		}
		if MATCHPLAYER2 >= (MATCH/2)+1 {
			break
		}
...
func printScores(scores [][][][]int) {
	...
	if MATCHPLAYER1 > MATCHPLAYER2 {
		fmt.Printf("PLAYER 1 WIN!!! %d MATCH(S) a %d\n", MATCHPLAYER1, MATCHPLAYER2)
	} else if MATCHPLAYER2 > MATCHPLAYER1 {
		fmt.Printf("PLAYER 2 WIN!!! %d MATCH(S) a %d\n", MATCHPLAYER2, MATCHPLAYER1)
	} else {
		fmt.Printf("EMPATE!!! P1 %d MATCH(S) a %d P2\n", MATCHPLAYER1, MATCHPLAYER2)
	}
...
// Define as configurações da partida.
func setConfiguration() {
	fmt.Println("Configurando a partida!")
	fmt.Println("Digite a quantidade de SCORES para definir um GAME: ")
	fmt.Scanln(&SCORE)
...
var SLEEPINGTIME = 8   // Valor entre 0 a 10 (define a espera entre cada goroutina)
